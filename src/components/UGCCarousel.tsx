...
export default function UGCCarousel() {
  ...
  return (
    <section className="ugc-section" style={{ background: "#121212", padding: "60px 0 80px", fontFamily: "system-ui, -apple-system, sans-serif", overflow: "hidden" }}>
      {/* Header */}
      <div style={{ textAlign: "center", marginBottom: 40, padding: "0 24px" }}>
        ...
        <h2
          ...
          style={{ fontSize: 40, fontWeight: 800, color: "#FFFFFF", letterSpacing: "-1px", margin: 0, opacity: titleVisible ? 1 : 0, transform: titleVisible ? "translateY(0)" : "translateY(40px)", transition: "opacity 0.7s ease, transform 0.7s ease" }}
        >
          {t("title")}
        </h2>
      </div>
      ...
      <div style={{ overflow: "hidden", width: "100%" }}>
        <div
          ...
          style={{ display: "flex", width: "max-content", willChange: "transform", animation: popup ? "none" : "ugc-slide 40s linear infinite" }}
          onMouseEnter={(e) => { if (!popup) (e.currentTarget as HTMLDivElement).style.animationPlayState = "paused"; }}
          onMouseLeave={(e) => { if (!popup) (e.currentTarget as HTMLDivElement).style.animationPlayState = "running"; }}
        >
          {allCards.map((card, i) => (
            <div
              key={i}
            ...
            >
              ...
              <div className="ugc-play-btn" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: 44, height: 44, borderRadius: "50%", background: "rgba(255,255,255,0.9)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <svg width="14" height="16" viewBox="0 0 14 16" fill="black"><path d="M0 0 L14 8 L0 16 Z" /></svg>
              </div>
              <div style={{ position: "absolute", right: 10, bottom: 60, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                <div style={{ textAlign: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 22 22" fill="white"><path d="M11 19.5C11 19.5 1 13 1 6.5A5 5 0 0 1 11 4.6 5 5 0 0 1 21 6.5C21 13 11 19.5 11 19.5Z" /></svg>
                  <div style={{ color: "#fff", fontSize: 10, marginTop: 2 }}>{card.likes}</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 22 22" fill="white"><path d="M2 2h18a1 1 0 0 1 1 1v11a1 1 0 0 1-1 1H6l-4 4V3a1 1 0 0 1 1-1Z" /></svg>
                  <div style={{ color: "#fff", fontSize: 10, marginTop: 2 }}>{card.comments}</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8" /><polyline points="16 6 12 2 8 6" /><line x1="12" y1="2" x2="12" y2="15" /></svg>
                  <div style={{ color: "#fff", fontSize: 10, marginTop: 2 }}>Share</div>
                </div>
              </div>
              <div style={{ position: "absolute", bottom: 12, left: 12, right: 50, display: "flex", flexDirection: "column", gap: 4 }}>
      ...
      {/* VIDEO POPUP */}
      {popup && (
        <div onClick={() => setPopup(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.88)", zIndex: 9998, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ position: "relative", background: "#111", borderRadius: 20, overflow: "hidden", maxWidth: 380, width: "100%", boxShadow: "0 24px 80px rgba(0,0,0,0.6)" }}>
            <button onClick={() => setPopup(null)} style={{ position: "absolute", top: 12, right: 12, zIndex: 10, background: "rgba(0,0,0,0.6)", border: "none", borderRadius: "50%", width: 36, height: 36, color: "#fff", fontSize: 18, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", backdropFilter: "blur(4px)" }}>×</button>
            ...
            <video src={popup.src} autoPlay controls playsInline style={{ width: "100%", maxHeight: "70vh", objectFit: "contain", display: "block", background: "#000" }} />
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        */
        ...
        @media (max-width: 767px) {
          .ugc-card { width: 160px !important; height: 284px !important; margin-right: 16px !important; }
          .carousel-track { animation-duration: 32s !important; }
        }
      `}</style>
    </section>
  );
