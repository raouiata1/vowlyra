...
export default function Demo() {
  ...
  return (
    <section id="demo" className="demo-section" style={{ background: "#121212", padding: "80px 40px", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <audio ref={audioRef} src={DEMO_AUDIO_URL} onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} onEnded={handleEnded} preload="metadata" />
      <div className="demo-grid" style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="demo-left">
          ...
          <h2 className="section-h2" style={{ fontSize: 40, fontWeight: 800, color: "#fff", lineHeight: 1.15, letterSpacing: "-1px", margin: 0, marginBottom: 12 }}>
          ...
          <div style={{ background: "rgba(29,185,84,0.08)", border: "1px solid rgba(29,185,84,0.25)", borderRadius: 12, padding: "16px 20px", marginBottom: 24 }}>
            <div style={{ color: "#1DB954", fontSize: 12, fontWeight: 700, letterSpacing: "0.5px", marginBottom: 10, textTransform: "uppercase" }}>{t("delivery_title")}</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              <div style={{ color: "#ccc", fontSize: 14, display: "flex", gap: 10, alignItems: "center" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1DB954" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                <span><strong style={{ color: "#fff" }}>{t("standard_label")}</strong> {t("standard_value")}</span>
              </div>
              <div style={{ color: "#ccc", fontSize: 14, display: "flex", gap: 10, alignItems: "center" }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1DB954" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
                <span><strong style={{ color: "#fff" }}>{t("express_label")}</strong> {t("express_value")}</span>
              </div>
            </div>
          </div>
          <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            {[t("check1"), t("check2"), t("check3")].map((item) => (
              <li key={item} style={{ color: "#ccc", fontSize: 14, display: "flex", gap: 10, alignItems: "flex-start" }}>
                <span style={{ color: "#1DB954", fontWeight: 700, marginTop: 1 }}>✓</span>{item}
              </li>
            ))}
          </ul>
        </div>
        ...
        <div style={{ background: "#181818", border: "1px solid #282828", borderRadius: 16, padding: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 28 }}>
            ...
            <div>
              <div style={{ color: "#fff", fontWeight: 700, fontSize: 16 }}>{t("track_name")}</div>
              <div style={{ color: "#999", fontSize: 13, marginTop: 3 }}>{t("track_sub")}</div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 28 }}>
            ...
            <button onClick={() => skipBy(10)} style={controlBtn}><SkipForwardSVG /></button>
          </div>
        </div>
      </div>
      <style>{`@keyframes wave { from { transform: scaleY(0.6); } to { transform: scaleY(1.2); } }`}</style>
    </section>
  );
}

const controlBtn: React.CSSProperties = { background: "none", border: "none", color: "#999", cursor: "pointer", padding: 8, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center", borderRadius: "50%", transition: "color 0.15s" };
